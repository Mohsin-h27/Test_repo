...
def post(event_external_id: str, data: dict) -> Optional[Dict]:
    ...
    Args:
        ...
        data (dict): A dictionary containing the supplier contact information, including:
            - supplier_contact_external_ids (list): A list of supplier contact external IDs to be added to the event
            - type (str): Object type, should always be "supplier_contacts"

    Returns:
        dict: The updated event data if successful, including the newly added supplier contacts. Returns None if:
...
def delete(event_external_id: str, data: dict) -> Optional[Dict]:
    ...
    Args:
        ...
        data (dict): A dictionary containing the supplier contact information, including:
            - supplier_contact_external_ids (list): A list of supplier contact external IDs to be removed from the event
            - type (str): Object type, should always be "supplier_contacts"

    Returns:
        dict: The updated event data if successful, with the specified supplier contacts removed. Returns None if:
            - The event does not exist
            - The event is not of type RFP
            - The operation fails
    """

...
def patch(id: str, new_options: List) -> Optional[Dict]:
    """Update a field options with given parameters.

    Args:
        id (Union[int, str]): The unique identifier of the field option to update.
        new_options (List): A list of new options to set for the field option.

    Returns:
        Optional[Dict]: The updated field option data if the update was successful,
            including any of the following fields:
            - type (str): Object type, should always be "fields"
            - field_id (str): Field identifier string
            - options (List[Dict]): List of options configured for the field
